...
RH_ASK driver; //Connect RF Receiver to Digital Pin No 10 of Arduino as it is default by the driver


int Sensor=9; //Connect PIR Sensor to Digital Pin No 9 of Arduino
...
//Tasks
Task t1(500, TASK_FOREVER, &t1Callback); //Task 1 runs every 500ms indefinitely
Task t2(1100, TASK_FOREVER, &t2Callback); //Task 2 runs every 1100ms indefinitely

Scheduler runner; //scheduler to execute each task
...
void t1Callback(){
    ...
    if(val==HIGH){
      captureImg(320, 240);
      Serial.write("Region:Tier-1 Node ID:2"); 
      delay(7500);
    }   
}
